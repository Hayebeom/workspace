...
public class FileDownloadView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
		...
		// application/octet-stream 은 알려지지 않은 파일의 타입을 지정할 때 이 타입을 사용한다
		// response.setHeader(name, value)는 응답메시지의 헤더정보를 설정할 때 사용한다
		// * 응답컨텐츠 타입, 응답컨텐츠 길이, 응답컨텐츠와 관련된 부가정보 설정
		// Content-Disposition으로 다운로드되는 파일의 이름을 브라우저에게 제공할 수 있다
		// value값의 attachment는 항상 파일을 다운로드하라는 것이다 (브라우저에 표시하지는 말고)
		// URLEncoder.encode(filename, "UTF-8") 는 파일이름에 한글이 포함되어 있을 경우 utf-8 방식으로 변환해서 파일명을 응답헤더에 포함시킨다

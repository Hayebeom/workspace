...
public class MybatisUtil {
	 ...
	 * 
	 *  - SqlSessionFactory
	 *  	쿼리 실행에 필요한 SqlSession객체를 제공하는 객체
	 *  	애플리케이션이 실행되는 동안 딱 한번만 생성해서 계속 사용
	 *  	SqlSession openSession()
	 *  		* 새로운 SqlSession 객체를 반환
	 *  
	 *  - SqlSessionFactoryBuilder
	 *  	mybatis 환경설정파일(/src/main/resources/mybatis/mybatis-config.xml)를 이용해서 
	...
	static {
		try {
			...
			// 스트림을 이용해서 mybatis 환경설정파일을 읽어서 SqlSessionFactoy 객체 획득
	...
	/**
	 * SQL 구문을 실행하는 SqlSession 객체를 반환
